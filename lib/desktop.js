...
function planSession(root, request) {
  ...
  const directory = sessionPath(root, id);
  return { directory, trace: path.join(directory, "input.kbmscript.json"), args: ["record", "--input", "keyboard-mouse", "--pid", String(request.pid), "--out", path.join(directory, "input.kbmscript.json"), "--start-delay", "5", ...(request.video ? ["--video", path.join(directory, "human.mp4")] : [])] };
}
...
async function start(args = {}) {
  ...
  const begin = async request => {
    ...
    try {
      ...
      send({ type: "focus", id: current.id, pid: request.pid, exe: request.exe, width: cursor?.width || 0, height: cursor?.height || 0 });
    } catch (error) {
      if (job !== current) return;
