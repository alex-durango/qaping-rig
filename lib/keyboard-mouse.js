...
function options(args) {
  ...
  return { maxSeconds, startDelayMs, countdownMs, showStatus: !args["no-status"], pid };
}
...
class DesktopInput extends EventEmitter {
  constructor(pid, mode, opts = {}) {
    ...
    this.child = spawn(opts.command || "powershell", opts.argv || ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", SHIM,
      ...
      "-CountdownMs", String(opts.countdownMs ?? 5000), ...(opts.showStatus ? ["-ShowStatus"] : []), ...(opts.waitForCapture ? ["-WaitForCapture"] : [])],
  ...
  try {
    ...
    const script = scripts.loadScript({ schema: scripts.SCHEMA, source: "recorded", created_at: new Date(started).toISOString(),
      ...
      meta: { recorded_from: "win32-rawinput", stopped_by: result.reason, mouse: "relative",
        cursor: "visible-client-pixels/v1", notes: "F9/F8 transport keys excluded" }, timeline });
...
async function play(args, script, runsRoot, reporter) {
  ...
  if (script.meta?.cursor !== "visible-client-pixels/v1" && !changes.some(e => e.input.cursor)) {
    warnings.push("This trace has no recorded menu cursor positions; mouse clicks may miss menu controls.");
  }
